...
```bash
./yii rabbitmq/publishers
./yii rabbitmq/publishers --json=1
./yii rabbitmq/middlewares
```
...
./yii rabbitmq/topology-apply --dryRun=0
./yii rabbitmq/topology-status
./yii rabbitmq/topology-status --json=1
```
...
- `illusiard\rabbitmq\components`
- `illusiard\rabbitmq\definitions`
- `illusiard\rabbitmq\message`
- `illusiard\rabbitmq\definitions\consume`
- `illusiard\rabbitmq\topology`
- `illusiard\rabbitmq\exceptions`
- documented console routes under `rabbitmq/*`
...
- выполните `rabbitmq/topology-apply --dryRun=0` или проверьте provisioning инфраструктуры
...
Env-флаги:
- `RABBITMQ_REQUIRED=1` (делает недоступный RabbitMQ ошибкой, удобно для CI)
